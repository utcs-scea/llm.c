...
// (taeklim)
__global__ void sim_repkv_forward_kernel1(floatX* replicated_qkv,
    ...
    replicated_qkv[idx_flat] = gqa_qkv[inp_idx];
}
...
void repkv_forward(floatX* out, const floatX* inp, int B, int T, int NH, int NH_KV, int HD, cudaStream_t stream) {
    ...
    if (replicate_factor > 1) {
        ...
        sim_repkv_forward_kernel1<<<num_blocks, block_size, 0, stream>>>(out, inp, B, T, NH, replicate_factor, HD);
#else
        repkv_forward_kernel1<<<num_blocks, block_size, 0, stream>>>(out, inp, B, T, NH, replicate_factor, HD);
#endif
    } else {
        printf("replicate_factor:%d\n", replicate_factor);
